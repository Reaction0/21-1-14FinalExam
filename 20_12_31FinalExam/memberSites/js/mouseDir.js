/*
	思路：通过判断鼠标移入移出时属于哪个部分的范围内，问题即可解决。
- 通过元素的两条对角线直线的斜率， 利用这两条直线和鼠标的位置与元素框中心点的位置进行比较得出三条直线的关系-
*/
// 制定元素的位置信息
const DIR_POS = {
  top: {
...
class MouseDir {
  ...
  judgeDir(ele, e) {
    ...
    // 第二个点：右下角坐标
    let x1 = pos.left + box_w,
      y1 = pos.top + box_h;


    // 中心点坐标
    let cx = pos.left + (box_w / 2),
      cy = pos.top + (box_h / 2);
  ...
  enter(dir) {
    ...
    // 启用过渡
    this.content_ele.style.transition = ".1s";
